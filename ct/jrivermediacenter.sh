...
echo -e "${INFO}${YW} Secure native VNC can be enabled from the Dashboard on port ${BGN}5902${CL}${YW} for Remmina, TigerVNC, and other compatible clients while UI mode is active; sign in with the same Dashboard username and password, with no client certificate import required.${CL}"
echo -e "${INFO}${YW} X2Go application mode can also be enabled from the Dashboard over SSH port ${BGN}22${CL}${YW}; sign in as ${BGN}jriver${CL}${YW} with the same Dashboard password and use the single-application command ${BGN}jrmc-x2go-launch${CL}${YW}.${CL}"

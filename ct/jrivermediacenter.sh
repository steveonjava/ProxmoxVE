...
echo -e "${INFO}${YW} VNC mode enables both browser noVNC and direct VNC on port ${BGN}5900${CL}${YW} together against the same JRMC session through one Xtigervnc backend. After Dashboard sign-in, use the same Dashboard password for VNC authentication.${CL}"

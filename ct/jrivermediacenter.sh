...
echo -e "${INFO}${YW} Native RDP can be enabled from the Dashboard on port ${BGN}3389${CL}${YW} for Remmina using user ${BGN}jriver${CL}${YW} and the same password as Dashboard.${CL}"
echo -e "${INFO}${YW} Native VNC can be enabled from the Dashboard on port ${BGN}5902${CL}${YW} for TigerVNC-compatible clients using ${BGN}TLSPlain${CL}${YW} and the same Dashboard credentials while UI mode is active.${CL}"

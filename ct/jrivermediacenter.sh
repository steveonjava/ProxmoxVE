...
echo -e "${INFO}${YW} Dashboard includes JRMC UI scale presets from ${BGN}100%${CL}${YW} to ${BGN}200%${CL}${YW}. VNC mode uses a larger server framebuffer as scale increases so browser noVNC can scale it back down cleanly.${CL}"

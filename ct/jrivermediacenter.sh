...
echo -e "${INFO}${YW} Dashboard includes JRMC UI scale presets from ${BGN}100%${CL}${YW} to ${BGN}200%${CL}${YW}. Changes persist for future noVNC, native VNC, and native RDP sessions and apply live best-effort to active sessions.${CL}"
echo -e "${INFO}${YW} Native RDP can be enabled from the Dashboard on port ${BGN}3389${CL}${YW} for Remmina using the same Dashboard username and password. Enabling it stops Media Server and the shared noVNC/native VNC UI session.${CL}"
echo -e "${INFO}${YW} Native VNC can be enabled from the Dashboard on port ${BGN}5902${CL}${YW} for TigerVNC-compatible clients using ${BGN}TLSPlain${CL}${YW} and the same Dashboard credentials while UI mode is active. Native VNC shares the browser UI session and is unavailable while RDP is active.${CL}"

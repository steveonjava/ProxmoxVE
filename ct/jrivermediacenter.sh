...
echo -e "${INFO}${YW} Secure native VNC can be enabled from the Dashboard on port ${BGN}5902${CL}${YW} for compatible desktop VNC clients, uses the same Dashboard credentials, and does not require client certificate import while UI mode is active.${CL}"

...
echo -e "${INFO}${YW} Secure native VNC can be enabled from the Dashboard on port ${BGN}5902${CL}${YW} for Remmina, TigerVNC, and similar desktop clients, using the same Dashboard credentials through an x11vnc shared-display backend while UI mode is active.${CL}"
